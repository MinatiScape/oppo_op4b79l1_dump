...
function lowram_device_setup()
{
    ...
    SeparateSoft=`getprop ro.separate.soft`
    if [ $MemTotal -lt 3145728 ]; then
        if [ "${SeparateSoft}" = "19321" ] || [ "${SeparateSoft}" = "19322" ] || [ "${SeparateSoft}" = "19323" ] || [ "${SeparateSoft}" = "19329" ]; then
...
function tranfer2TfCard(){
    ...
    #cp /data/vendor/camera
    mkdir -p ${newpath}/spdebug
    touch ${newpath}/spdebug/.nomedia
    mv data/vendor/camera/* ${newpath}/spdebug/
...
function copyWeixinXlog(){
    ...
    if [ "${saveallxlog}" = "${argtrue}" ]; then
        ...
        cp -rf ${XLOG_DIR}/*.xlog ${newpath}/xlog/
    else
        if [ -d "${XLOG_DIR}" ]; then
            mkdir -p ${newpath}/xlog
...
function getSystemSatus() {
    ...
    then
        ...
        dumpsys batterystats -c > ${outputPath}/battersystats_for_bh.txt
        setprop sys.tranfer.finished mv:batterystats
        dumpsys location > ${outputPath}/location.txt
...
function save_latest_log() {
    ...
    if [ -d ${LATEST_KENEL_LOG_PATH} ]; then
        LOGTIME=`date +%F-%H-%M-%S`
        CAMERA_LOG_DIR_KENELLOG_DIR=/sdcard/oppo_log/camera_monkey_log/kenel_log_${LOGTIME}
